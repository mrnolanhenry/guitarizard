...
const App = () => {
	...
	const checkActiveTuning = (instrument: FrettedInstrument): Tuning => {
		...
		return commonTuningMatch ?? new Tuning(instrument.name, Constants.CUSTOM, tuningNotes);
	};
	...
	const setInstrumentTuningToPreset = (tuning: Tuning): void => {
		...
		fretBoard.courses.forEach((course, index) => fretBoard.setCourseTuningNote(course.id, tuning.notes[index]));
