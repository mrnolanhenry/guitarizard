...
const KeySearchSelector = (props: IKeySearchSelectorProps) => {
  const [searchTimeout, setSearchTimeout] = useState(null as ReturnType<typeof setTimeout> | null);
  ...
  const {
   ...
   const handleInputChange = (event: React.SyntheticEvent, inputValue: string) => {
    ...
    const isInsertingText: boolean = !!(event.nativeEvent as InputEvent).inputType && (event.nativeEvent as InputEvent).inputType === "insertText";
   ...
   const handleFilterOptions = (inputValue: string, isInsertingText: boolean): Key[] => {
    if (!inputValue) {
      setPotentialKeys(allKeys);
      return [];
    }
      ...
      let filterOptions = potentialKeys.filter((key) => isKeyDisplayNameMatch(key, trimVal));
      
      if (filterOptions.length === 0) {
        filterOptions = potentialKeys.filter((key) => isNoteMatch(key, inputValues));
      }
