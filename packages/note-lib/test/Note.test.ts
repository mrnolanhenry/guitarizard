...
tap.test("class Note", function (t) {
  ...
  t.equal(naturalNote.isSimilar(naturalNote), true, "detect similar (self) - natural");
  t.equal(naturalNote.isSimilar(new Note("Z", NotePitch.Neither)), false, "dismiss non-similar notes");
  ...
  t.equal(sharpNote.findFlatOrNatural(), Yb, "find flat or natural note in aliases");
